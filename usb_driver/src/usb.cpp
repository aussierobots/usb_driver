...
#include <sstream>
#include <iomanip>
#include "usb_driver/callback.hpp"
...
Connection::Connection(
  int vendor_id, uint16_t product_id, std::string serial_str,
  int log_level)
{
...
void Connection::init()
{
  ...
  if (vendor_id_ == 0 || product_id_ == 0) {
    throw std::string(
      "Vendor ID and Product ID must be set to non-zero values for USB initialization");
  }
  ...
  // Register hotplug callbacks - at preent we handle only one
  std::vector<uint16_t> product_ids = {product_id_};
  hp_attach_.resize(product_ids.size());
  for (size_t i = 0; i < product_ids.size(); i++) {
    rc = libusb_hotplug_register_callback(
      ctx_, LIBUSB_HOTPLUG_EVENT_DEVICE_ARRIVED, LIBUSB_HOTPLUG_ENUMERATE, vendor_id_,
      product_ids[i], class_id_, hotplug_attach_callback_fn, NULL, &hp_attach_[i]);
    if (LIBUSB_SUCCESS != rc) {
      throw std::string("Error registering hotplug attach callback for product ID: ") +
            std::to_string(product_ids[i]);
    }
  }
  ...
  hp_detach_.resize(product_ids.size());
  for (size_t i = 0; i < product_ids.size(); i++) {
    rc = libusb_hotplug_register_callback(
      ctx_, LIBUSB_HOTPLUG_EVENT_DEVICE_LEFT, LIBUSB_HOTPLUG_ENUMERATE, vendor_id_,
      product_ids[i], class_id_, hotplug_detach_callback_fn, NULL, &hp_detach_[i]);
    if (LIBUSB_SUCCESS != rc) {
  ...
  char * serial_num_string)
{
  ...
  for (ssize_t i = 0; i < deviceCount; i++) {
    ...
    if (rc < 0) {
      // User-programmed iSerial may not be reliable
...
bool Connection::open_device()
{
  ...
  auto product_ids = std::vector<uint16_t>{product_id_};
  devh_ = open_device_with_serial_string(
    ctx_, vendor_id_, product_ids, serial_str_,
    serial_num_string);
  if (!devh_) {
